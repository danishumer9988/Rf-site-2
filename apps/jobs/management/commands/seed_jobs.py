...
class Command(BaseCommand):
    ...
    def _create_rich_jobs(self, count, user, categories):
        ...
        for i in range(count):
            ...
            # Find the category object. Fail loudly instead of silently
            # defaulting to categories[0] -- a silent fallback here is what
            # causes jobs to be mislabeled with the wrong category.
            category = next((c for c in categories if c.name == template['category']), None)
            if category is None:
                raise ValueError(
                    f"Seed template category '{template['category']}' does not match "
                    f"any existing Category. Fix the template or create the category first."
                )
